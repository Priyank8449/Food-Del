...
export const acceptOrder = async (req, res) => {
    try {
        ...
        if (assignment.status !== "broadcasted") {
            return res.status(500).json({ message: "assignment  is expired" })
        ...
        if (allreadyAssigned) {
            return res.status(500).json({ message: "you are allready assigned to  an order" })
        ...
        return res.status(500).json({ message: "  accept order err", error: error.message })
